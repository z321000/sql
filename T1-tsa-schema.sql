...
COMMENT ON COLUMN booking.booking_id IS
    'surrogate key added to replace BOOKING composite
PK';
...
COMMENT ON COLUMN booking.member_id IS
    'Unique member id across TSA for member who
made this booking';
...
ALTER TABLE booking
    ADD CONSTRAINT uq_booking_previous_pk UNIQUE ( resort_id,
                                                   cabin_no,
                                                   booking_from );
...
ALTER TABLE cabin
    ADD CONSTRAINT ck_cabin_nobedrooms CHECK ( cabin_nobedrooms >= 1
                                               AND cabin_nobedrooms <= 4 );

ALTER TABLE cabin
    ADD CONSTRAINT ck_cabin_bathroom_type CHECK ( upper(cabin_bathroom_type) IN ( upper
    ('I'), upper('C') ) );
