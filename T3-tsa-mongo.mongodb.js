...
db.resortsRecord.find({"avg_temperature.summer":{"$gt":25}},{"resorts.company_name":"Tropical Dream"});
...
db.resortsRecord.updateMany({"resorts.company_name":"Tropical Dream"},
{"$set":{"resorts.$[].company_name":"Tropical Heaven"}});

db.resortsRecord.find({"resorts.company_name":"Tropical Heaven"});
...
if ({"$eq":[db.resortsRecord.find({"name":"Mount Isa ,QLD","location.latitude":-20.7256,"location.longitude":139.4927}).limit(1).size(),1]}) {
  db.resortsRecord.updateOne({"name":"Mount Isa ,QLD","location.latitude":-20.7256,"location.longitude":139.4927},
                                {"$push":{"resorts":{"id":11,"name":"Amazing Resort",
                                                    "address":"2001 Princess Road",
                                                    "phone":"0320180924",
                                                    "year_built":2023,
                                                    "company_name":"Australia Experience"}}})
} else {
  db.resortsRecord.insertOne({"_id":7,"name":"Mount Isa ,QLD","location":{"latitude":-20.7256,"longitude":139.4927},"avg_temperature":{"summer":31.9,"winter":17.3},"no_of_resorts":1,"resorts":[]})
}


db.resortsRecord.find({"resorts.id":11});
